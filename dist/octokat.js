...
var Chainer, OBJECT_MATCHER, Replacer, TREE_OPTIONS, parseQueryString, plus, toPromise, _ref,
  __slice = [].slice;
...
parseQueryString = function(link, acc) {
  var key, pair, val, _i, _len, _ref1, _ref2, _results;
  if (/\{\?/.test(link)) {
    return;
  }
  if (/\?/.test(link)) {
    ...
    if (link.length > 0) {
      ...
      for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
        pair = _ref1[_i];
        _ref2 = pair.split('='), key = _ref2[0], val = _ref2[1];
        _results.push(acc[decodeURIComponent(key)] = decodeURIComponent(val));
      }
      return _results;
    }
  }
